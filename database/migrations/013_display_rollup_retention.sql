...
-- Exact and display rollups are two non-destructive views of the same events.
-- Expire them in the same transaction so readers and readiness verification
-- can never observe one retention window without the other.
CREATE OR REPLACE FUNCTION purge_expired_trace_data(
...
BEGIN
    ...
    WHERE groups.last_seen_at < aggregate_cutoff
      AND NOT EXISTS (
          SELECT 1
      ...
      AND NOT EXISTS (
          ...
          WHERE rollups.project_id = groups.project_id
